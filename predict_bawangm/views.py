...
import pickle
import pandas as pd
from predict_bawangm.forms import SymptomForm
import os

with open(os.path.join(settings.BASE_DIR, 'static', 'ML', 'bm_model_plant_conditions.pkl'), 'rb') as f:
    df_plant_conditions = pickle.load(f)

with open( os.path.join(settings.BASE_DIR, 'static', 'ML', 'bm_model_diseases.pkl'), 'rb') as f:
    df_diseases = pickle.load(f)

with open(os.path.join(settings.BASE_DIR, 'static', 'ML', 'bm_model_rules.pkl'), 'rb') as f:
